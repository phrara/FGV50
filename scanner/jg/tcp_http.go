...
func TcpHTTP(ret *tools.Result, inurl string) bool {
...
func httpIdentifyResult(ret *tools.Result, inurl string) (map[string]interface{}, error) {
	timeout := ret.TTL
...
func defaultRequests(Url string, timeout int) ([]RespLab, error) {
	...
	resp, err := connection.ConnProxyHttp(req, timeout)
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
	...
	if len(regexp.MustCompile("30").FindAllStringIndex(responseStatusCode, -1)) == 1 {
		...
		resp, _ := connection.ConnProxyHttp(req, timeout)
...
func identify(url string, timeout int) ([]IdentifyResult, error) {
	...
	for _, rule := range tools.RuleData {
