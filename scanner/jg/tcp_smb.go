...
func TcpSMB(ret *tools.Result) bool {
	timeout := ret.TTL
	host := ret.Host
	port := ret.Port
	if smbPublic(ret, host, port, timeout) {
		return true
	}
...
func smbPublic(ret *tools.Result, host string, port int, timeout int) bool {
...
func RandomBytes(numBytes int) []byte {
	...
	_ = binary.Read(crand.Reader, binary.BigEndian, &randBytes)
	return randBytes
}
